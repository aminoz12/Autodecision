-- Garage accounts: "paiement en compte" + payment terms per garage.
--
--  * clients.payment_terms_days  — 7 / 10 / 15 / 30 days, chosen when the
--    magasin creates the garage (default 30).
--  * orders.mode_paiement         — ESPECES / CARTE / VIREMENT / CHEQUE /
--    EN_COMPTE (nullable for older rows). EN_COMPTE is reserved to garages:
--    nothing is cashed at creation, the amount goes to the garage account.
--  * orders.echeance              — due date of an on-account order, set by
--    trigger from the garage's payment terms (also when a garagiste quote is
--    accepted, from the acceptance date).
--  * create_order_with_lines      — accepts p_payload.mode_paiement; garagiste
--    portal orders are always EN_COMPTE.

alter table public.clients
  add column if not exists payment_terms_days integer not null default 30;
alter table public.clients drop constraint if exists clients_payment_terms_days_check;
alter table public.clients
  add constraint clients_payment_terms_days_check
  check (payment_terms_days in (7, 10, 15, 30));

alter table public.orders add column if not exists mode_paiement text;
alter table public.orders drop constraint if exists orders_mode_paiement_check;
alter table public.orders
  add constraint orders_mode_paiement_check
  check (mode_paiement is null or mode_paiement in ('ESPECES', 'CARTE', 'VIREMENT', 'CHEQUE', 'EN_COMPTE'));
alter table public.orders add column if not exists echeance date;

create index if not exists orders_client_account_idx
  on public.orders (organization_id, client_id, mode_paiement)
  where mode_paiement = 'EN_COMPTE';

-- ---------------------------------------------------------------------
-- Due date of on-account orders
-- ---------------------------------------------------------------------
create or replace function public.orders_set_echeance()
returns trigger
language plpgsql
...
declare
  ...
  v_base date;
begin
  if new.mode_paiement is distinct from 'EN_COMPTE' or new.client_id is null then
    return new;
  end if;
  -- A pending quote is not due yet; the due date starts when it is accepted.
  if new.devis then
    new.echeance := null;
    return new;
  end if;
  -- An explicit due date is kept; otherwise (or when the quote is accepted /
  -- the garage changes) it is derived from the garage's payment terms.
  if new.echeance is null
     or (tg_op = 'UPDATE' and old.devis and not new.devis)
     or (tg_op = 'UPDATE' and old.client_id is distinct from new.client_id) then
    select c.payment_terms_days into v_terms
    from public.clients c where c.id = new.client_id;
    v_base := case when tg_op = 'INSERT' then new.date_commande
                   else timezone('Europe/Paris', now())::date end;
    new.echeance := v_base + coalesce(v_terms, 30);
  end if;
  return new;
end;
$$;

drop trigger if exists orders_set_echeance on public.orders;
create trigger orders_set_echeance
  before insert or update of mode_paiement, client_id, devis, echeance
  on public.orders
  for each row execute function public.orders_set_echeance();

-- ---------------------------------------------------------------------
-- create_order_with_lines: + mode_paiement
-- ---------------------------------------------------------------------
...
begin
  ...
  insert into public.orders (
    ...
    statut_livreur, consigne, workflow_status, bl, date_bl, mode_paiement
  ) values (
    v_org, v_ref,
...
-- ---------------------------------------------------------------------
-- Backfill: existing garage orders are, by nature, on account.
-- ---------------------------------------------------------------------
update public.orders o
set mode_paiement = 'EN_COMPTE',
    echeance = case when o.devis then null else o.date_commande + c.payment_terms_days end
from public.clients c
where c.id = o.client_id
  and c.is_garage
  and o.mode_paiement is null
  and o.is_restock = false;
